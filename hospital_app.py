...
try:
    ...
    st.error("Model file not found. Place 'hospital_model (1).pkl' in the app directory.")
    st.stop()
...
st.markdown(
    ...
    <div style="background:linear-gradient(135deg, #1e3a8a 0%, #1a56db 60%, #0ea5e9 100%);
        ...
        <div style="font-size:14px;font-weight:500;color:rgba(255,255,255,0.7);text-transform:uppercase;letter-spacing:0.1rem;margin-bottom:12px;">
...
with st.form("triage_form"):
    ...
    st.markdown(
        ...
        <div style="background:#f0f9ff;border:1px solid #bae6fd;border-radius:14px;padding:20px 24px;margin-bottom:20px;">
            <div style="display:flex;align-items:center;gap:10px;margin-bottom:4px;">
                <span style="background:#0284c7;color:white;border-radius:8px;padding:4px 10px;font-size:12px;font-weight:600;">2</span>
    ...
    st.markdown(
        ...
        <div style="background:#fff7ed;border:1px solid #fed7aa;border-radius:14px;padding:20px 24px;margin-bottom:20px;">
            <div style="display:flex;align-items:center;gap:10px;margin-bottom:4px;">
                ...
                <span style="font-size:16px;font-weight:600;color:#0c4a6e;">How severe are your symptoms?</span>
            </div>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    st.markdown(
        ...
        <div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:14px;padding:20px 24px;margin-bottom:20px;">
            <div style="display:flex;align-items:center;gap:10px;margin-bottom:4px;">
                <span style="background:#059669;color:white;border-radius:8px;padding:4px 10px;font-size:12px;font-weight:600;">4</span>
    ...
    st.markdown(
        ...
        <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:14px;padding:20px 24px;margin-bottom:20px;">
            <div style="display:flex;align-items:center;gap:10px;margin-bottom:4px;">
                <span style="background:#475569;color:white;border-radius:8px;padding:4px 10px;font-size:12px;font-weight:600;">5</span>
...
if submitted:
    ...
    st.markdown(
        f"""
        <div style="background:{info['bg']};border:1.5px solid {info['border']};border-radius:16px;padding:28px 32px;">
            <div style="font-size:44px">{info['icon']}</div>
            <div style="font-size:24px;font-weight:700;color:{info['color']};margin-top:8px;">{dept_name}</div>
            <div style="font-size:16px;color:#374151;margin-top:6px;">Confidence: {confidence:.1f}%</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    with res_col:
        ...
        st.markdown(
            f"""
            <div style="background:{info['bg']};border:1.6px solid {info['border']};border-radius:16px;padding:28px 32px;">
                <div style="font-size:44px">{info['icon']}</div>
    ...
    with prob_col:
        ...
        sorted_depts = sorted(dept_map_inv.items(), key=lambda item: proba[item[0]], reverse=True)
        bars_html = ""
        for idx, dname in sorted_depts:
            ...
            <div style="margin-bottom:14px;">
                ...
                <div style="background:#f3f4f6;border-radius:6px;height:8px;overflow:hidden;">
                    <div style="background:{'linear-gradient(90deg, ' + dinfo['color'] + ', ' + dinfo['border'] + ')' if is_top else '#e5e7eb'};height:100%;border-radius:6px;width:{pct}%;transition:width 0.5s ease;"></div>
                </div>
            </div>
            """
